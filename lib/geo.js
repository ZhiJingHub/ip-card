...
export async function getGeoInfo(ip) {
  ...
  // 定义多个数据源及其解析规则
  const sources = [
    // --- 源 1: ip-api.com (速度快，无 HTTPS，后端调用没问题) ---
    async () => {
      const res = await fetch(`http://ip-api.com/json/${ip}?fields=status,countryCode,city,isp`, { signal: AbortSignal.timeout(3000) });
    ...
    // --- 源 2: ipwho.is (免费，无需 Key，支持 HTTPS) ---
    async () => {
  ...
  for (const fetcher of sources) {
    try {
      ...
      // 当前源失败，继续尝试下一个...
      console.warn('Geo Source Failed, trying next...');
    }
  }
